...
describe("control-plane phase-0 baseline", () => {
  ...
  it("builds local control-plane services with capability discovery", async () => {
    ...
    try {
      ...
      const createdWorkflow = await services.workflowService.create({
        definition: {
          steps: [
            {
              id: "seed",
      ...
      expect(createdWorkflow.id.length).toBeGreaterThan(0);
      const listedWorkflows = await services.workflowService.list({ limit: 10 });
      expect(listedWorkflows.items[0]?.id).toBe(createdWorkflow.id);
      ...
      await expect(
        services.workflowService.create({
          definition: {
            ...
            dependencies: [{ from: "seed", to: "missing-step" }]
          }
        })
      ).rejects.toMatchObject({ code: "CONFIG_ERROR" });
    } finally {
  ...
  it("resumes failed workflow runs from the first failed node", async () => {
    const dir = mkdtempSync(join(tmpdir(), "athena-workflow-resume-"));
    try {
      const config = loadConfig(dir);
      ...
      const backend: ExecutionBackend = {
        kind: "local",
        async run(request) {
          const stepId = request.metadata?.workflowStepId ?? "unknown-step";
          const attempt = (attemptsByStep.get(stepId) ?? 0) + 1;
          attemptsByStep.set(stepId, attempt);
          if (stepId === "review" && attempt === 1) {
            throw new Error("review failed on first attempt");
          }
          return {
            sessionId: request.sessionId,
            output: `ok:${stepId}:${attempt}`,
            provider: request.provider ?? "mock",
            model: request.model ?? "mock-model",
            createdAt: new Date().toISOString()
          };
      ...
      const directive = await services.directiveService.create({
        input: "workflow step input"
      });
      const harnessProfile = await services.harnessProfileService.create({
        displayName: "Resume Harness",
        version: "v1",
        config: {
          provider: "mock",
          model: "mock-model",
      ...
      const firstRun = await services.workflowService.resume(workflow.id);
      expect(firstRun.status).toBe("failed");
      expect(firstRun.stepStates.seed?.status).toBe("ok");
      expect(firstRun.stepStates.review?.status).toBe("failed");

      const secondRun = await services.workflowService.resume(workflow.id);
      expect(secondRun.id).toBe(firstRun.id);
      expect(secondRun.status).toBe("ok");
      expect(secondRun.stepStates.seed?.status).toBe("ok");
      expect(secondRun.stepStates.review?.status).toBe("ok");
      expect(secondRun.stepStates.review?.attempt).toBe(2);

      const observability = await services.workflowService.status(workflow.id);
      expect(observability.workflow.id).toBe(workflow.id);
      expect(observability.run.id).toBe(secondRun.id);
      expect(observability.progress.totalSteps).toBe(2);
      expect(observability.progress.completedSteps).toBe(2);
      expect(observability.artifactRefs.length).toBe(2);
      expect(observability.eta.source).toBe("completed");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });

  it("captures workflow evidence via control-plane validation against active runs", async () => {
    const dir = mkdtempSync(join(tmpdir(), "athena-workflow-evidence-"));
    try {
      let activeRun:
        | {
            sessionId: string;
      ...
      const services = createLocalControlPlaneServices({ config, executionBackend: backend });
      const directive = await services.directiveService.create({
        input: "evidence workflow step"
      });
      const harnessProfile = await services.harnessProfileService.create({
        displayName: "Evidence Harness",
        version: "v1",
        config: {
          provider: "mock",
          model: "mock-model",
      ...
      const run = await services.workflowService.resume(workflow.id);
      expect(run.status).toBe("ok");

      const evidenceDir = join(dir, ".athena", "run-evidence");
      const runDirs = readdirSync(evidenceDir, { withFileTypes: true }).filter((entry) => entry.isDirectory());
      expect(runDirs.length).toBe(1);
      const evidenceFiles = readdirSync(join(evidenceDir, runDirs[0]!.name)).filter((name) => name.endsWith(".json"));
      expect(evidenceFiles.length).toBe(1);
      const evidence = JSON.parse(
        readFileSync(join(evidenceDir, runDirs[0]!.name, evidenceFiles[0]!), "utf8")
      ) as {
        label: string;
        type: string;
        createdAt: string;
      };
      expect(evidence.label).toBe("step-log");
      expect(evidence.type).toBe("text");
      expect(typeof evidence.createdAt).toBe("string");

      const observability = await services.workflowService.status(workflow.id);
      expect(
        observability.run.executionLog.some((entry) => entry.message.includes("Evidence attached:") && entry.stepId === "evidence-step")
      ).toBe(true);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }

...
describe("control-plane policy and fleet services", () => {
  ...
  it("returns local fleet summary counters in FleetSummary contract shape", async () => {
    const dir = mkdtempSync(join(tmpdir(), "athena-control-plane-fleet-summary-"));
    try {
      const config = loadConfig(dir);
      const services = createLocalControlPlaneServices({ config });
      ...
      const summary = await services.fleetService.getSummary();
  ...
  it("maps k8s fleet metrics and capabilities through backend provider support", async () => {
    const dir = mkdtempSync(join(tmpdir(), "athena-control-plane-fleet-k8s-"));
    try {
      const backend: ExecutionBackend = {
        kind: "k8s",
        async run(request) {
  ...
  it("returns aggregated k8s CPU/memory usage through fleet service", async () => {
    const dir = mkdtempSync(join(tmpdir(), "athena-control-plane-fleet-k8s-resources-"));
    try {
      const backend: ExecutionBackend = {
        kind: "k8s",
        async run(request) {
  ...
  it("selects k8s metrics provider via config when requested", async () => {
    const dir = mkdtempSync(join(tmpdir(), "athena-control-plane-fleet-provider-select-"));
    try {
      const backend: ExecutionBackend = {
        kind: "local",
        async run(request) {
  ...
  it("caches fleet operational summaries for 5 seconds before recomputing", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-02-20T00:00:00.000Z"));
    const dir = mkdtempSync(join(tmpdir(), "athena-control-plane-fleet-cache-"));
    try {
      let activeRuns = [
        {
          sessionId: "s1",
      ...
      const first = await services.fleetService.getSummary();

...
The control plane is the central orchestration layer of Team Orchestrator. It exposes the core functionality via a REST API and ensures consistent behavior across local API, trusted-server, console, and CLI-facing paths.
...
- **App-state repositories (`app-state/`)**: Manage durable control-plane state with SQLite as the local default and server-ready repository boundaries.

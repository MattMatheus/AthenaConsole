...
function readModelProvider(value) {
  ...
  const providerKind = readRequiredString(provider.providerKind, "modelProvider.providerKind");
  const baseUrl = readRequiredString(provider.baseUrl, "modelProvider.baseUrl").replace(/\/+$/, "");
  const defaultModel = readRequiredString(provider.defaultModel, "modelProvider.defaultModel");
...
async function callOpenAiCompatibleProvider(provider, inputs) {
  ...
  if (!response.ok) {
    throw new Error(`Model provider request failed: ${response.status} ${response.statusText} ${body.slice(0, 800)}`);
  }
